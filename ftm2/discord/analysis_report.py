...
def render_analysis_message(state, details_by_symbol: Dict[str, List]) -> str:
    ...
    for sym, details in details_by_symbol.items():
        ...
        rvp = ind.get("rv_pr"); rvp_txt = "—" if rvp is None else f"{rvp:.3f}"
        parts.append(f"• 이유: 모멘텀 {c.get('momentum',0):+.2f}, 돌파 {c.get('breakout',0):+.2f}, 평균회귀 {c.get('meanrev',0):+.2f} | 레짐 {best.regime}, RV%tile {rvp_txt} {'✅' if all([gates.get('regime_ok'),gates.get('rv_band_ok')]) else '⚠️'}")
        # 계획/안전장치
        plan = best.plan
