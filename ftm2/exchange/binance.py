...
class BinanceClient:
    ...
    def get_account(self) -> Dict[str, Any]:
        """Return raw account response."""
        return self._http_request("GET", "/v2/account", signed=True)

    # Backwards compat: alias used by newer callers
    ...
    def account_snapshot(self) -> Dict[str, Any]:
        ...
        if not isinstance(acc, dict):
            return {}
        try:
            equity = float(acc.get("totalMarginBalance") or 0.0)
            wallet = float(acc.get("totalWalletBalance") or 0.0)
            upnl = float(acc.get("totalUnrealizedProfit") or 0.0)
            avail = float(acc.get("availableBalance") or 0.0)
        except Exception:
